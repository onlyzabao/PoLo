    ...
    for rule in rules:
        ...
        if rule[lb_idx[0] : rb_idx[0] + 1] == "(X,Y)":
            ...
            if len(lb_idx) == 3:  # Rule length 2
                path.extend(
                    [rule[lb_idx[1] : lb_idx[1] + 5], rule[lb_idx[2] : lb_idx[2] + 5]]
                )
                body1 = rule_body(
                    rule, "(X,A)", "(A,X)", path, s_idx, lb_idx, rb_idx, ", "
                )
                body2 = rule_body(
                    rule, "(A,Y)", "(Y,A)", path, s_idx, lb_idx, rb_idx, "\n"
                )
                ...
                path.extend(
                    [
                        rule[lb_idx[1] : lb_idx[1] + 5],
                        rule[lb_idx[2] : lb_idx[2] + 5],
                        rule[lb_idx[3] : lb_idx[3] + 5],
                    ]
                )
                if ("(X,A)" in path) or ("(A,X)" in path):
                    ...
                    body2 = rule_body(
                        rule, "(A,B)", "(B,A)", path, s_idx, lb_idx, rb_idx, ", "
                    )
                    body3 = rule_body(
                        rule, "(B,Y)", "(Y,B)", path, s_idx, lb_idx, rb_idx, "\n"
                    )
                elif ("(X,B)" in path) or ("(B,X)" in path):
                    body1 = rule_body(
                        rule, "(X,B)", "(B,X)", path, s_idx, lb_idx, rb_idx, ", "
                    )
                    body2 = rule_body(
                        rule, "(B,A)", "(A,B)", path, s_idx, lb_idx, rb_idx, ", "
                    )
                    body3 = rule_body(
                        rule, "(A,Y)", "(Y,A)", path, s_idx, lb_idx, rb_idx, "\n"
                    )
...
def rule_body(
    rule, correct_order, incorrect_order, path, s_idx, lb_idx, rb_idx, add_string
):
    """
    ...
    if correct_order in path:
        ...
        if rule[s_idx[idx + 1] + 1] == "_":
            body = (
                rule[s_idx[idx + 1] + 2 : lb_idx[idx + 1]] + correct_order + add_string
            )
        else:
            body = (
                "_"
                + rule[s_idx[idx + 1] + 1 : lb_idx[idx + 1]]
                + correct_order
                + add_string
            )
    else:
...
def create_rule_list(rules):
    ...
    rule_list = sorted(
        rule_list, key=lambda x: x[0], reverse=True
...
def filter_rule_dict(dictionary, regex_pattern):
    pattern = re.compile(regex_pattern)
    return {key: value for key, value in dictionary.items() if pattern.match(key)}
